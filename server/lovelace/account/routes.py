...
def create_account():
    ...
    if not new_email or not new_password or email_validation(new_email) or password_validation(new_password):  # check if empty input
        logger.info("%s Did not succeed in creating an account due to failed input validation", request.remote_addr)
        ...
        try:
            ...
            logger.info("%s Succeed in creating an account using email %s", request.remote_addr, new_email)
            return jsonify(
                {"creation": True, "response": "Account was created successfully"}
            )
        except db_errors.DuplicateKeyError:
            logger.info("%s Did not succeed in creating an account due to duplicated email %s", request.remote_addr, new_email)
            return jsonify({"creation": False, "response": "Email already exist"})

        except argon2_exceptions.VerifyMismatchError:
            logger.info("%s Did not succeed in creating an account due to argon2 exceptions mismatch", request.remote_addr)
...
def login_account():
    ...
    if not email or not password or email_validation(email) or password_validation(password):  # check if empty input
        account_collection = mongo.account
        logger.info("%s Failed to login using email %s due to failed input validation", request.remote_addr, email)
        return jsonify({"login": False, "response": "Invalid email or password"})
    account_collection = mongo.account
    try:
    ...
    if valid_login:
        ...
        logger.info("%s Logged in successfully using email %s", request.remote_addr, email)
        return jsonify(
